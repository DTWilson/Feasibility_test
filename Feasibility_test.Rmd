...
get_ocs_pc <- function(cs, n, x0, x1, n_e, n_t)
{
  ...
  #   sub_a$a <- sub_a$a*(1- pbinom(f, 2*n, sub_a[,3]))
    ...
    # while(opt_a$convergence != 0){
    #   k <- runif(2)
    #   print(k)
    #   opt_a <- optim(2, fn=get_oc_pc, #lower = c(0.1,0.1), upper=c(1,1),
    #                  cs=cs, n=n, xi=x0, tI=T, s=s, f=f, a=a, n_e=n_e, n_t=n_t)#, method="L-BFGS-B")
    # }
    ...
    # while(opt_b$convergence != 0){
    #   k <- runif(2)
    #   print(k)
    #   opt_b <- optim(k, fn=get_oc_pc,# lower = c(0.1,0.1), upper=c(1,1),
    #              cs=cs, n=n, xi=x1, tI=F, s=s, f=f, a=a, n_e=n_e, n_t=n_t)#, method="L-BFGS-B")
    # }
...
ggplot(df[df$n_t==234 & df$null_pow==0.6,], aes(a, b, colour=as.factor(np), linetype=t)) + geom_line() + 
