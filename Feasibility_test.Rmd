...
Rather than defining hypotheses wrt power for a fixed design, can we look at sample size required for a fixed power?

```{r}
mu <- 0.3
n_max <- 500; n_min <- 312

get_fup <- function(phi, nm, mu, n_e, n_t)
{
  ...
  exp_n <- get_exp_N(n_e, n_t=nm, p_r)

  (2*sd^2 + mu^2 *p_a*(1-p_a))*(qnorm(0.9) + qnorm(0.975))/((p_a*mu)^2*exp_n)
}

# Null
df <- expand.grid(p_r=seq(0,1,0.01), p_a=seq(0,1,0.01))
df$p_f <- apply(df, 1, get_fup, nm=n_max, mu=mu, n_e = 800, n_t =235)
sub <- df

# Alternative
df <- expand.grid(p_r=seq(0,1,0.01), p_a=seq(0,1,0.01))
df$p_f <- apply(df, 1, get_fup, nm=n_min, mu=mu, n_e = 800, n_t =235)
sub_a <- df
...
```{r}
sub <- df[df$null_pow == 0.65 & df$n_t == "257.4" & df$np == 50,]
...
To do - worked example relating to a real pilot trial.
